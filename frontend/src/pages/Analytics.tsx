  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-white">Institutional Analytics & ML Diagnostics</h1>
        ...
        <button
          onClick={loadData}
          className="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-300 text-xs font-semibold self-start sm:self-auto transition-all"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <MetricCard
          ...
          subtitle={`${overview?.high_risk_count || 0} students alert`}
          icon={AlertTriangle}
          variant="rose"
        />
        <MetricCard
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-panel p-6 rounded-3xl border border-slate-800">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-base font-bold text-white">Attendance vs Marks vs Dropout Risk</h2>
                ...
                <Tooltip
                  ...
                  content={({ active, payload }) => {
                    if (active && payload && payload.length) {
                      ...
                      return (
                        <div className="p-3 rounded-xl bg-slate-900 border border-slate-700 text-xs text-white shadow-xl">
                          <p className="font-bold text-emerald-400">{data.name} ({data.student_id})</p>
                          <p className="text-slate-300">Dept: {data.department}</p>
                          <p className="text-slate-300">Attendance: {data.attendance}%</p>
                          <p className="text-slate-300">Marks: {data.marks}%</p>
                          <p className="font-bold mt-1 text-rose-400">Risk Score: {data.risk_score}% ({data.risk_level})</p>
                        </div>
                      );
                    }
                    return null;
        ...
        <div className="glass-panel p-6 rounded-3xl border border-slate-800">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-base font-bold text-white">Department Risk Distribution</h2>
              <p className="text-xs text-slate-400">Aggregated cohorts across all engineering disciplines</p>
            </div>
          </div>
        ...
        <div className="glass-panel p-6 sm:p-8 rounded-3xl border border-slate-800 space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-800">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-white">Machine Learning Model Diagnostics & Validation</h2>
                <p className="text-xs text-slate-400">
                  Calculated metrics from test cohort ({modelPerf.test_samples} evaluation samples)
                </p>
              </div>
            </div>

            <span className="text-xs font-mono px-3 py-1 rounded-full bg-slate-900 border border-slate-700 text-slate-400">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="p-4 rounded-2xl bg-slate-900/80 border border-slate-800 text-center">
              <p className="text-xs font-bold text-slate-400 uppercase">Calculated Accuracy</p>
              <p className="mt-1 text-3xl font-black font-mono text-emerald-400">
                {(modelPerf.accuracy * 100).toFixed(1)}%
              </p>
            </div>

            <div className="p-4 rounded-2xl bg-slate-900/80 border border-slate-800 text-center">
              <p className="text-xs font-bold text-slate-400 uppercase">Weighted Precision</p>
              <p className="mt-1 text-3xl font-black font-mono text-teal-400">
                {(modelPerf.precision * 100).toFixed(1)}%
              </p>
            </div>

            <div className="p-4 rounded-2xl bg-slate-900/80 border border-slate-800 text-center">
              <p className="text-xs font-bold text-slate-400 uppercase">Weighted Recall</p>
              <p className="mt-1 text-3xl font-black font-mono text-cyan-400">
            ...
            <div className="p-4 rounded-2xl bg-slate-900/80 border border-slate-800 text-center">
              ...
              <p className="mt-1 text-3xl font-black font-mono text-indigo-400">
                ...
                <table className="w-full text-center text-xs font-mono">
                  <thead>
                    <tr className="text-slate-400 border-b border-slate-800">
                      ...
                      <th className="p-2 text-emerald-400">Pred: LOW</th>
                      <th className="p-2 text-amber-400">Pred: MED</th>
                      <th className="p-2 text-rose-400">Pred: HIGH</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-800">
                    {modelPerf.confusion_matrix.map((row, i) => (
                      <tr key={i}>
                        <td className="p-2.5 text-left font-bold text-slate-300">
                ...
                {modelPerf.feature_importances.map((item, idx) => (
                  <div key={idx} className="space-y-1">
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="font-mono text-emerald-400">{(item.importance * 100).toFixed(1)}%</span>
                    </div>
